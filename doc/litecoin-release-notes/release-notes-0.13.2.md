Faithcoin Core version 0.13.2 is now available from:
  ...
  <https://download.newcoin.org/newcoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a newcoin
...
This allows running newcoind without having to do any manual configuration.
...
`newcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /newcoinrpc>
        ...
        # optional bypass newcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< newcoinrpc:<password>
    </Location>
...
Restarting newcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in newcoin.conf
...
For example:
    ...
    $ src/newcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `newcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `newcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of newcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of newcoins being spent is exactly
...
- `newcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `newcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
